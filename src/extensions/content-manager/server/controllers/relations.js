...
module.exports = {
  async find(ctx) {
    ...
    if(target.uid=="plugin::users-permissions.user" && model == "api::partner-timeslot.partner-timeslot"){
        query.filters = {
          $and: [
            {
              "role": {
    ...
    if(target.uid=="api::product.product"){
      query.filters = {
        $and: [
          {
            publishedAt: {
    ...
    // console.log("----->", {query: query.filters, uid: target.uid})
    ...
    ctx.body = entities.map(pick(pickFields)).map(o=>{
